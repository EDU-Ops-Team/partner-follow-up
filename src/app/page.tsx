...
function SiteExpansion({ siteId }: { siteId: Id<"sites"> }) {
  ...
  const draftByClassificationId = new Map(drafts.map((draft) => [String(draft.classificationId), draft]));
    ...
    <div className="mt-4 border-t border-gray-100 pt-4 space-y-2">
      ...
      {items.map((item) => {
        if (item.kind === "message") {
          const c = item.data;
          const relatedDraft = draftByClassificationId.get(String(c._id));
          const href = relatedDraft ? `/review/${relatedDraft._id}` : `/inbound/${c._id}`;
          const actionLabel = relatedDraft ? "Open draft review" : "Review inbound";
          return (
            <Link
              key={`msg-${c._id}`}
              href={href}
              className="block rounded border border-transparent bg-gray-50 px-3 py-2 text-sm transition-all hover:border-blue-200 hover:bg-blue-50"
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 text-xs font-medium text-blue-700">{actionLabel} -&gt;</div>
                </div>
              </div>
            </Link>
          );
        ...
        return (
          <Link
            key={`draft-${d._id}`}
            href={`/review/${d._id}`}
            className="block rounded border border-transparent bg-yellow-50 px-3 py-2 text-sm transition-all hover:border-yellow-300 hover:bg-yellow-100"
          >
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-2 text-xs font-medium text-amber-800">Open draft review -&gt;</div>
              </div>
            </div>
          </Link>
        );
